...
def _summarize(node: str, update: dict) -> tuple[str, dict | None]:
    ...
    if node == "quality_check":
        ...
        return (f"Quality check: {verdict} (score {q.get('score')})", {
            "score": q.get("score"),
            "passed": q.get("passed"),
            "issues": q.get("issues", []),
...
async def run_workflow(session_id: str, *, resume: bool = False) -> None:
    ...
    async with SessionLocal() as db:
        ...
        if session is None:
            logger.warning("run requested for missing session", extra={"ctx_session": session_id})
            return
        ...
        try:
            async for chunk in engine.graph.astream(
                ...
                for node, update in chunk.items():
                    ...
                    event = await repo.add_event(
                        db, session_id, node=node, status="completed",
            ...
            await pubsub.publish(session_id, {"type": "done", "status": STATUS_COMPLETED})
            ...
            await repo.update_session_status(db, session_id, STATUS_FAILED, error=str(exc))
            await repo.add_event(
                db, session_id, node="workflow", status="failed", message=str(exc)
            )
            await pubsub.publish(
                session_id, {"type": "error", "status": STATUS_FAILED, "message": str(exc)}
            )
